...
def serialize_agent(agent: Shift3Agent, role: str) -> Dict:
    return {
        ...
        'wins': int(agent.wins), 'losses': int(agent.losses),
        'draws': int(agent.draws), 'total_moves': int(agent.total_moves),
        'surround_wins': int(agent.surround_wins),
...
def deserialize_agent(data: Dict, player_id: int) -> Shift3Agent:
    agent = Shift3Agent(player_id=player_id,
                        mcts_sims=data.get('mcts_sims', 20),
                        minimax_depth=data.get('minimax_depth', 2))
...
def create_agents_zip(agent1: Shift3Agent, agent2: Shift3Agent,
                       config: Dict) -> io.BytesIO:
    buf = io.BytesIO()
    with zipfile.ZipFile(buf, 'w', zipfile.ZIP_DEFLATED) as zf:
        zf.writestr('agent1.json',
                    json.dumps(serialize_agent(agent1, 'P1'), indent=2))
        zf.writestr('agent2.json',
                    json.dumps(serialize_agent(agent2, 'P2'), indent=2))
...
def load_agents_from_zip(uploaded_file) -> Tuple:
    try:
        with zipfile.ZipFile(uploaded_file, 'r') as zf:
            names = zf.namelist()
            if not all(f in names
                       for f in ['agent1.json','agent2.json','config.json']):
...
# ============================================================================
# Sidebar — Sliders (wider range, defaults 20 sims / depth 2)
# ============================================================================

st.sidebar.header("⚙️ Shift-3 Controls")

with st.sidebar.expander("1. Agent 1 (Red) Parameters", expanded=True):
    lr1    = st.slider("Learning Rate α₁",   0.01, 1.0,  0.25, 0.01)
    gamma1 = st.slider("Discount γ₁",        0.50, 0.999, 0.97, 0.001,
                       format="%.3f")
    mcts1  = st.slider("MCTS Simulations₁",  5, 1000, 20, 5)
    mm1    = st.slider("Minimax Depth₁",     1, 16,   2,  1)
    temp1  = st.slider("Temperature₁",       0.0, 3.0, 1.0, 0.1)

with st.sidebar.expander("2. Agent 2 (Blue) Parameters", expanded=True):
    lr2    = st.slider("Learning Rate α₂",   0.01, 1.0,  0.25, 0.01)
    gamma2 = st.slider("Discount γ₂",        0.50, 0.999, 0.97, 0.001,
                       format="%.3f")
    mcts2  = st.slider("MCTS Simulations₂",  5, 1000, 20, 5)
    mm2    = st.slider("Minimax Depth₂",     1, 16,   2,  1)
    temp2  = st.slider("Temperature₂",       0.0, 3.0, 1.0, 0.1)

with st.sidebar.expander("3. Training Configuration", expanded=True):
    episodes    = st.number_input("Training Episodes",    10, 100000, 500,  50)
...
if 's3_agent1' not in st.session_state:
    st.session_state.s3_agent1 = Shift3Agent(
        1, lr1, gamma1, mcts_sims=mcts1, minimax_depth=mm1)
    st.session_state.s3_agent2 = Shift3Agent(
        2, lr2, gamma2, mcts_sims=mcts2, minimax_depth=mm2)
...
with col4:
    ...
    st.metric("MCTS P1/P2", f"{mcts1}/{mcts2}")
...
if 's3_training_history' in st.session_state and \
        st.session_state.s3_training_history:
    ...
    if th.get('episode') and len(th['episode']) > 0:
        ...
        with st.expander("📋 Full Training Data Table"):
            df_th = pd.DataFrame({
                'Episode': th['episode'],
                'P1 Wins': th['agent1_wins'],
                'P2 Wins': th['agent2_wins'],
                'Draws':   th['draws'],
...
if len(agent1.policy_table) > 3 or len(agent1.q_table) > 3:
    if st.button("▶️ Watch Championship Match", use_container_width=True):
        ...
        with st.spinner("Agents competing..."):
            while not champ.game_over and mn < 80:
                ...
                fig_b = draw_shift3_board(
                    ...
                    f"Move {mn}: {pname} — {act.description()}",
                    last_action=act, win_cells=champ.win_cells,
                    move_count=mn)
                board_ph.pyplot(fig_b)
                plt.close(fig_b)

                if champ.move_history:
        ...
        if champ.winner == 1:
            st.success("🏆 Agent 1 (Red) Wins via Surround!")
        elif champ.winner == 2:
            st.error("🏆 Agent 2 (Blue) Wins via Surround!")
        else:
...
with col_h1:
    human_col = st.selectbox("You Play As",
                              ["Player 1 (Red)", "Player 2 (Blue)"])
with col_h2:
    ai_opp = st.selectbox("AI Opponent Brain",
                           ["Agent 1 (Red)", "Agent 2 (Blue)"])
with col_h3:
    ai_str = st.selectbox("AI Strength",
                           ["Easy (10 sims)",   "Medium (50 sims)",
                            "Hard (150 sims)",  "Expert (400 sims)",
                            "Master (800 sims)"])
    s_map = {"Easy (10 sims)": 10,    "Medium (50 sims)": 50,
             "Hard (150 sims)": 150,  "Expert (400 sims)": 400,
             "Master (800 sims)": 800}

if st.button("🎯 Start Human vs AI Game", use_container_width=True,
             type="primary"):
    ...
    st.session_state.s3_ai_ref     = \
        agent1 if "Agent 1" in ai_opp else agent2
...
if st.session_state.get('s3_hv_active', False):
    ...
    if hv_g.game_over:
        if hv_g.winner == h_pid:
            ...
            st.error("🤖 AI Wins — it sandwiched your piece!")
        else:
            st.warning("🤝 Draw! (Position looped or max moves)")
    else:
        turn = ("Your Turn ✍️" if hv_g.current_player == h_pid
                else "AI Thinking... 🤖")
        pstr = "Red (P1)" if hv_g.current_player == 1 else "Blue (P2)"
        hcur = hv_g.hand[hv_g.current_player]
        st.info(
            f"**{turn}** | {pstr} | Hand: {hcur} pieces"
            f" | Move #{hv_g.move_count + 1}")
    ...
    with col_board:
        fig_hv = draw_shift3_board(
            ...
            "Human vs AI — Shift-3",
            last_action=last_act, win_cells=hv_g.win_cells,
    ...
    with col_side:
        ...
        st.metric("P1 In Hand",
                  f"{bi['p1_hand']} / {Shift3Game.PIECES_PER_PLAYER}")
        st.metric("P2 In Hand",
                  f"{bi['p2_hand']} / {Shift3Game.PIECES_PER_PLAYER}")
        ...
        rep = bi['seen_max']
        if rep >= 2:
            st.warning(f"⚠️ Board repeated {rep}× — draw at 3×!")
        elif rep == 1:
            st.caption("Position seen 1× — avoid repetition")

        st.markdown(
            f"**Move Count**: {bi['move_count']} / {Shift3Game.MAX_MOVES}")

        # Surround threat indicators
        p1_pos = bi['p1_pos']
        p2_pos = bi['p2_pos']
        if len(p1_pos) == 2 and len(p2_pos) >= 1:
            ...
            if pe - ps == 2:
                mid = (ps + pe) // 2
                if hv_g.board[mid] == 2:
                    st.error("⚡ P1 has an active SURROUND threat!")
                elif hv_g.board[mid] == 0:
                    st.warning("P1 waiting for opponent to enter the trap")
            elif pe - ps == 3:
        ...
        if len(p2_pos) == 2 and len(p1_pos) >= 1:
            ...
            if pe - ps == 2:
                mid = (ps + pe) // 2
                if hv_g.board[mid] == 1:
                    st.error("⚡ P2 has an active SURROUND threat!")
                elif hv_g.board[mid] == 0:
                    st.warning("P2 waiting for opponent to enter the trap")
            elif pe - ps == 3:
    ...
    if not hv_g.game_over and hv_g.current_player == h_pid:
        ...
        if place_acts:
            ...
            for ci, act in enumerate(place_acts):
                ...
                if cols[ci % len(cols)].button(
                        lbl, key=f"s3_place_{act.position}_{hv_g.move_count}"):
        ...
        if slide_acts:
            ...
            for ci, act in enumerate(slide_acts):
                ...
                sym  = "←" if act.direction == -1 else "→"
                lbl  = f"{hint}: [{act.position}]{sym}[{dest}]"
                if cols[ci % len(cols)].button(
                        lbl, key=f"s3_slide_{act.position}_{act.direction}_{hv_g.move_count}"):
                    hv_g.make_action(act)
                    st.session_state.s3_last_action = act
                    st.rerun()

        if not place_acts and not slide_acts:
            st.warning("No valid actions — game may be stuck!")

    # Action history chart during play
...
with st.expander("🔬 Quick AI Simulation (No Training Effect)"):
    sim_n = st.slider("Games to simulate", 5, 300, 20, 5)
    if st.button("▶️ Run Simulation", key="s3_sim"):
        ...
        with st.spinner(f"Simulating {sim_n} games..."):
            for _ in range(sim_n):
                g_tmp = Shift3Game()
                ag    = {1: agent1, 2: agent2}
                while not g_tmp.game_over:
                    cur = g_tmp.current_player
                    a   = ag[cur].choose_action(g_tmp, training=False)
                    if a is None:
                        break
                    g_tmp.make_action(a)
                sim_r[g_tmp.winner] += 1
                total_moves_acc += g_tmp.move_count

        avg_moves = total_moves_acc / max(1, sim_n)
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("🔴 P1 Wins",   sim_r[1],    f"{sim_r[1]/sim_n:.1%}")
        c2.metric("🔵 P2 Wins",   sim_r[2],    f"{sim_r[2]/sim_n:.1%}")
        c3.metric("🤝 Draws",     sim_r[None], f"{sim_r[None]/sim_n:.1%}")
        c4.metric("Avg Moves",    f"{avg_moves:.1f}")
        ...
        while not demo_g.game_over:
            ...
            a   = (agent1 if cur == 1 else agent2).choose_action(
                demo_g, training=False)
            if a is None:
                break
...
### Starting Position
Both players begin with **2 pieces in hand** — none pre-placed.

### On Your Turn: Choose ONE of
1. **PLACE** — put one piece from hand onto any **empty** square
2. **SLIDE** — move one of your **already-placed** pieces exactly **one square**
   left or right into an adjacent **empty** square

### Winning Condition
**SURROUND**: achieve the pattern **[Your piece — Opponent piece — Your piece]**
in 3 consecutive squares. You are sandwiching the opponent.

### Draw Conditions
- The same board position (board + hands + whose turn) appears **3 times**
- **120 moves** pass without a winner

### Strategic Principles
- 🎯 **Close the brackets**: Your 2 pieces must flank an opponent piece
- 🔀 **Slides are free**: sliding costs nothing from hand — use them aggressively
- 🛡️ **Don't walk into the gap**: if opponent's pieces are 2 apart, avoid the middle
- ⚡ **Placement phase**: place pieces to pressure both halves of the board
- 🔄 **Avoid loops**: 3 repetitions = draw — always have a commitment direction
- 🧮 **3⁵ = 243 states**, but piece-can-move → loops → deep tactical depth

### AI Architecture
MCTS (PUCT) + Negamax/Alpha-Beta (move-ordered) + Q-Learning + Policy Table.
The heuristic evaluator models surround distance, gap dynamics, opponent
sandwiching, mobility asymmetry, edge penalties, and repetition-loop avoidance.
Loop-detection feeds back into prior generation to discourage drawn positions.
""")
